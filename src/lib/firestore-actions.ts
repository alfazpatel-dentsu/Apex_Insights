...
const resolveDocRef = (db: Firestore, collectionName: string, id?: string) => {
    const trimmed = id?.toString().trim();
    return trimmed ? doc(db, collectionName, trimmed) : doc(collection(db, collectionName));
};

export const saveActionItem = async (db: Firestore, data: Partial<ActionItem>, id?: string) => {
    const ref = resolveDocRef(db, 'actionItems', id);
...
export const saveKpiData = async (db: Firestore, kpiData: Omit<KpiData, 'id'>, weeklyData: Omit<KpiWeeklyData, 'id' | 'kpiDataId'>[], existingKpiId?: string) => {
    const batch = writeBatch(db);
    const kpiDocRef = resolveDocRef(db, 'kpis', existingKpiId);
    const kpiId = kpiDocRef.id;
    
    const payload = stripUndefined({ 
        ...kpiData, 
        uploadRecordId: kpiId,
        kpiType: kpiData.kpiType || 'PRIMARY',
        currency: kpiData.currency || 'INR',
        direction: kpiData.direction || 'ASC',
        cluster: kpiData.cluster || 'Unassigned',
        lob: kpiData.lob || 'N/A',
        cduLead: kpiData.cduLead || 'N/A',
        emCsm: kpiData.emCsm || 'N/A',
        type: kpiData.type || 'Performance',
    });
    batch.set(kpiDocRef, payload, { merge: true });

    weeklyData.forEach(week => { 
        const weeklyId = `${kpiId}_w${week.weekOfMonth}`;
        batch.set(doc(db, 'kpiWeeklyData', weeklyId), stripUndefined({ 
            ...week, 
            kpiDataId: kpiId, 
            month: kpiData.month,
...
export const updateWeeklyComment = async (db: Firestore, id: string, comment: string) => {
    if (!id) throw new Error('Weekly comment update requires a document id');
    const ref = doc(db, 'kpiWeeklyData', id);
    try {
      await updateDoc(ref, { comment: comment || "" });
    } catch (e) { 
...
export const saveMonthlySpend = async (db: Firestore, data: Omit<MonthlySpend, 'id'>, id?: string) => {
    const ref = resolveDocRef(db, 'monthlySpends', id);
    const payload = stripUndefined({
        ...data,
        ...
        type: data.type || 'N/A',
        subEntity: data.subEntity || 'N/A',
        channelVendor: data.channelVendor || 'N/A',
        creditLine: data.creditLine || 'N/A',
        currency: data.currency || 'INR',
        team: data.team || 'N/A',
        month: data.month || '',
        actualSpendsInr: sanitizeNumber(data.actualSpendsInr),
...
export const saveWeeklySpend = async (db: Firestore, data: Omit<WeeklySpend, 'id'>, id?: string) => {
    const ref = resolveDocRef(db, 'weeklySpends', id);
    const month = parseMonthStr(data.week, true) || format(new Date(), 'yyyy-MM');
    const payload = stripUndefined({
        ...data,
        clientId: data.clientId || 'N/A',
        brandName: data.brandName || 'Unknown',
        ...
        currency: data.currency || 'INR',
        team: data.team || 'N/A',
        week: data.week || format(new Date(), 'dd-MM-yyyy'),
        month,
        spendsInr: sanitizeNumber(data.spendsInr),
        uploadRecordId: ref.id,
    });
    try {
        await setDoc(ref, payload, { merge: true });
...
export const saveLead = async (db: Firestore, data: Omit<Lead, 'id'>, id?: string) => {
  const ref = resolveDocRef(db, 'leads', id);
