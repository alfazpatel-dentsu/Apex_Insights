...
export function AddUserDialog({ isOpen, onOpenChange, onSave }: AddUserDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto rounded-[2rem] glass">
        <DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[10px] font-black uppercase tracking-widest opacity-60">Full Name</FormLabel>
                   <FormControl>
                      <Input className="rounded-xl bg-foreground/5 border-none" placeholder="e.g. John Doe" {...field} disabled={isSaving} />
                    </FormControl>
                  <FormMessage />
                </FormItem>
              )}
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[10px] font-black uppercase tracking-widest opacity-60">Official Email ID</FormLabel>
                   <FormControl>
                      <Input 
                        className="rounded-xl bg-foreground/5 border-none" 
                        type="email" 
                      ...
                      <FormField
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem
                              key={item.id}
                              className="flex flex-row items-start space-x-3 space-y-0 rounded-xl bg-foreground/5 p-3"
                            >
                              <FormControl>
                                <Checkbox
            ...
            <DialogFooter className="pt-6">
                <Button type="button" variant="ghost" className="rounded-xl" onClick={() => onOpenChange(false)} disabled={isSaving}>Cancel</Button>
                <Button type="submit" className="rounded-xl font-bold px-6" disabled={isSaving}>
