    ...
    <div className="flex flex-1 flex-col gap-8 pb-10">
      <PageHeader title="WEEKLY WBR" description="Collaborative workspace for account engagement and risk review.">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 rounded-[1.5rem] bg-white/40 dark:bg-white/5 p-2 backdrop-blur-md shadow-inner border border-white/20">
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-2xl" onClick={() => setCurrentWbrDate(subWeeks(currentWbrDate, 1))}><ChevronLeft className="h-5 w-5" /></Button>
            ...
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-2xl" onClick={() => setCurrentWbrDate(addWeeks(currentWbrDate, 1))}><ChevronRight className="h-5 w-5" /></Button>
          </div>
          <Button variant="outline" className="h-14 px-6 rounded-3xl glass border-none gap-2 font-bold shadow-lg" onClick={handleExport}><FileSpreadsheet className="h-5 w-5 text-primary" /> Export</Button>
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white/30 dark:bg-black/20 p-4 rounded-[2.5rem] backdrop-blur-3xl border border-white/10 shadow-2xl"><div className="relative flex-1 min-w-[200px]"><Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/60" /><Input placeholder="Search account name..." className="pl-12 rounded-2xl glass border-none h-12 text-sm shadow-inner" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
      ...
      {isLoading ? (<div className="flex flex-col items-center justify-center p-32 gap-4"><Loader2 className="animate-spin h-10 w-10 text-primary/40" /><span className="text-xs font-black uppercase tracking-widest opacity-40">Accessing Aztec Intelligence...</span></div>) : filteredClients.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in duration-700">{filteredClients.map(client => (<WbrClientCard key={client.uniqueId} client={client} entry={wbrEntries?.find(e => e.clientId === client.uniqueId)} onClick={() => router.push(`/dashboard/wbr/${client.uniqueId}?date=${format(currentWbrDate, 'yyyy-MM-dd')}`)} />))}</div>) : (<div className="flex flex-col items-center justify-center p-40 glass-card border-dashed"><Zap className="h-12 w-12 text-primary/20 mb-4" /><h3 className="text-xl font-bold font-headline">No Accounts Found</h3><p className="text-sm text-muted-foreground">Adjust filters or ensure KPI records are being synchronized.</p></div>)}
    </div>
  );
}
...
function FilterGroup({ label, value, onChange, options, isRag }: { label: string, value: string, onChange: (v: string) => void, options: string[], isRag?: boolean }) {
  return (
    <div className="flex items-center gap-2 bg-white/40 dark:bg-white/5 rounded-2xl p-1 px-4 backdrop-blur-md shadow-inner border border-white/20"><span className="text-[10px] font-black uppercase tracking-widest opacity-40">{label}:</span><Select value={value} onValueChange={onChange}><SelectTrigger className="h-8 min-w-[80px] border-none bg-transparent shadow-none text-[10px] font-black uppercase p-0 focus:ring-0"><SelectValue /></SelectTrigger><SelectContent className="rounded-xl glass border-none"><SelectItem value="all" className="text-[10px] font-bold">ALL</SelectItem>{options.map(opt => (<SelectItem key={opt} value={opt} className={cn("text-[10px] font-bold uppercase", isRag && opt === 'Red' && 'text-destructive', isRag && opt === 'Green' && 'text-success', isRag && opt === 'Amber' && 'text-warning')}>{opt}</SelectItem>))}</SelectContent></Select></div>
  );
}

function WbrClientCard({ client, entry, onClick }: { client: Client, entry?: WbrEntry, onClick: () => void }) {
  return (
    <Card className="glass-card border-none ios-shadow cursor-pointer transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl group overflow-hidden relative" onClick={onClick}><div className="absolute top-0 right-0 p-5 opacity-0 group-hover:opacity-100 transition-opacity"><div className="h-10 w-10 rounded-2xl bg-primary/10 flex items-center justify-center text-primary"><ArrowRight className="h-5 w-5" /></div></div>
      ...
      <CardContent className="space-y-4"><div className="h-[1px] bg-foreground/5" /><div className="grid grid-cols-2 gap-4"><div className="space-y-1"><span className="text-[8px] font-black uppercase opacity-40">Contract</span><span className={cn("block text-[10px] font-bold", entry?.contractStatus === 'Expired' && 'text-destructive', entry?.contractStatus === 'Negotiation' && 'text-warning')}>{entry?.contractStatus || 'N/A'}</span></div><div className="space-y-1"><span className="text-[8px] font-black uppercase opacity-40">Billing</span><span className="block text-[10px] font-bold truncate">{entry?.financeIssues ? 'Issue Reported' : 'Clear'}</span></div></div><div className="pt-2"><span className="text-[8px] font-black uppercase opacity-40 block mb-1">Strategic Summary</span><p className="text-[11px] leading-relaxed line-clamp-2 font-medium opacity-70 italic">{entry?.summary || 'No review entry for this week.'}</p></div></CardContent>
    </Card>
  );
}
