...
export function WbrEditDialog({ isOpen, onOpenChange, onSave, client, entry, userRole, isAdmin, isWindowOpen, wbrDate }: WbrEditDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto rounded-[3rem] glass">
        <DialogHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            {!isWindowOpen && !isAdmin && (
              <div className="flex items-center gap-2 bg-destructive/10 text-destructive px-3 py-1.5 rounded-xl border border-destructive/20">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pt-4">
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-1">
                   <h4 className="text-[10px] font-black uppercase tracking-widest opacity-40">Account Configuration (Admin Only)</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-[2rem] bg-foreground/[0.03] border border-foreground/5">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input className="rounded-xl bg-background/50 border-none h-10" {...field} disabled={!canEditField(f)} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-1">
                   <h4 className="text-[10px] font-black uppercase tracking-widest opacity-40">EM / CSM Updates</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-[2rem] bg-foreground/[0.03] border border-foreground/5">
                    <FormField
                      control={form.control}
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value} disabled={!canEditField('contractStatus')}>
                            <FormControl><SelectTrigger className="rounded-xl bg-background/50 border-none"><SelectValue /></SelectTrigger></FormControl>
                    ...
                    <div className="md:col-span-2">
                        ...
                        render={({ field }) => (
                            ...
                            <FormControl><Textarea className="rounded-xl bg-background/50 border-none min-h-[60px]" placeholder="Add billing context..." {...field} disabled={!canEditField('financeIssues')} /></FormControl>
                            </FormItem>
                        )}
                        />
                    </div>
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea className="rounded-xl bg-background/50 border-none" placeholder="Growth leads..." {...field} disabled={!canEditField('organicOpportunities')} /></FormControl>
                        </FormItem>
                      )}
                    />
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Textarea className="rounded-xl bg-background/50 border-none" placeholder="Service expansion..." {...field} disabled={!canEditField('crossSellOpportunities')} /></FormControl>
                        </FormItem>
                      )}
                    />
                </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 px-1">
                   <h4 className="text-[10px] font-black uppercase tracking-widest opacity-40">Cluster Lead / Ops Review</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-[2rem] bg-foreground/[0.03] border border-foreground/5">
                    <FormField
                      control={form.control}
                    ...
                    <div className="md:col-span-2">
                        ...
                        render={({ field }) => (
                            ...
                            <FormControl><Textarea className="rounded-xl bg-background/50 border-none min-h-[100px]" placeholder="Strategic summary for the week..." {...field} disabled={!canEditField('summary')} /></FormControl>
                            </FormItem>
                        )}
                        />
                    </div>
