    ...
    <>
      ...
      <div className="max-h-[300px] overflow-y-auto space-y-1 custom-scrollbar">
        {filtered.length > 0 ? filtered.map(option => (
          <div 
            key={option} 
            className="flex items-center gap-2 p-2 rounded-xl hover:bg-foreground/5 cursor-pointer text-xs font-bold" 
  ...
  const renderGainerLoserList = (gainers: GainerLoser[], losers: GainerLoser[]) => (
    <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-foreground/5">
      <div className="space-y-2">
        ...
        )) : <span className="text-[9px] italic opacity-40">No gains</span>}
      </div>
      <div className="space-y-2">
        ...
        )) : <span className="text-[9px] italic opacity-40">No losses</span>}
      </div>
    </div>
  );
    ...
    <div className="flex flex-1 flex-col gap-8 pb-10">
      <PageHeader title="SPENDS DASHBOARD" description="Strategic analytical insights and spending trends.">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Popover>
              <PopoverTrigger asChild>
                <button className={cn("flex items-center gap-2 h-9 px-4 rounded-xl glass border-none text-[10px] font-black uppercase tracking-widest transition-all", selectedChannels.length > 0 ? "bg-primary text-white" : "text-foreground/60 hover:text-foreground")}>
                  <Filter className="h-3 w-3" />
                  Channel {selectedChannels.length > 0 && `(${selectedChannels.length})`}
                </button>
              </PopoverTrigger>
              <PopoverContent className="w-[280px] p-2 rounded-2xl glass border-none shadow-2xl" align="start">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className={cn("flex items-center gap-2 h-9 px-4 rounded-xl glass border-none text-[10px] font-black uppercase tracking-widest transition-all", selectedClients.length > 0 ? "bg-primary text-white" : "text-foreground/60 hover:text-foreground")}>
                  <Filter className="h-3 w-3" />
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className={cn("flex items-center gap-2 h-9 px-4 rounded-xl glass border-none text-[10px] font-black uppercase tracking-widest transition-all", selectedTeams.length > 0 ? "bg-primary text-white" : "text-foreground/60 hover:text-foreground")}>
                  <Filter className="h-3 w-3" />
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className={cn("flex items-center gap-2 h-9 px-4 rounded-xl glass border-none text-[10px] font-black uppercase tracking-widest transition-all", selectedTypes.length > 0 ? "bg-primary text-white" : "text-foreground/60 hover:text-foreground")}>
                  <Filter className="h-3 w-3" />
          ...
          <div className="flex items-center gap-2 bg-white/40 dark:bg-white/5 rounded-2xl p-1 backdrop-blur-md shadow-inner border border-white/20">
            <span className="text-[10px] font-black uppercase tracking-widest pl-3 opacity-50">Year:</span>
            <Select value={selectedYear} onValueChange={setSelectedYear}>
              <SelectTrigger className="h-8 w-24 rounded-xl glass border-none text-[10px] font-black uppercase tracking-widest focus:ring-0">
                <SelectValue />
              </SelectTrigger>
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Select value={dimension} onValueChange={(v) => setDimension(v as Dimension)}>
            <SelectTrigger className="h-8 w-24 rounded-xl glass border-none text-[10px] font-black uppercase">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="rounded-xl glass border-none">
              {DIMENSIONS.map(d => (
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl text-primary" onClick={onExport}>
