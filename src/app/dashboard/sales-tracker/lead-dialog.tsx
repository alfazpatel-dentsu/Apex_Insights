          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 rounded-[2rem] bg-foreground/[0.03] border border-foreground/5">
                <FormField control={form.control} name="companyName" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input className="rounded-xl bg-background/50 border-none h-12 shadow-inner px-4 font-bold" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                ...
                <FormField control={form.control} name="estimatedValue" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="number" className="rounded-xl bg-background/50 border-none h-12 shadow-inner px-4 font-mono font-bold" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="email" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="email" className="rounded-xl bg-background/50 border-none h-12 shadow-inner px-4 font-bold" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                    ...
                    <FormField
                      ...
                      render={({ field }) => {
                        return (
                          <FormItem
                            key={service}
                            className="flex flex-row items-center space-x-3 space-y-0 rounded-xl bg-foreground/[0.03] p-3 border border-transparent hover:border-primary/20 transition-all cursor-pointer"
                          >
                            <FormControl>
                              <Checkbox
            ...
            <FormField control={form.control} name="notes" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea className="rounded-[2rem] bg-foreground/[0.03] border-none min-h-[120px] shadow-inner p-6 text-sm font-medium leading-relaxed resize-none" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <DialogFooter className="pt-6 border-t border-foreground/5">
                ...
                <Button type="submit" className="rounded-xl h-12 px-10 font-black shadow-xl shadow-primary/20 uppercase tracking-widest text-[10px]">
                  Save Lead
                </Button>
            </DialogFooter>
          </form>
