...
export default function BusinessSnapshotPage() {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      {!stats ? (
        <div className="p-32 border border-dashed border-ink flex flex-col items-center justify-center text-center space-y-6 bg-white">
        ...
        <div ref={snapshotRef} id="snapshot-content" className="space-y-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 bg-white border border-ink shadow-2xl p-10 space-y-8">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[9px] font-black uppercase tracking-[0.3em] opacity-40">WEEKLY SPENDS PULSE</span>
                  <h2 className="text-3xl font-black tracking-tighter uppercase mt-1">12-Week Momentum</h2>
                </div>
                <div className="flex items-center gap-6"><div className="h-2 w-2 rounded-full bg-destructive" /><span className="text-[9px] font-black uppercase tracking-widest opacity-40">SPEND</span></div>
              </div>
            ...
            <div className="bg-white border border-ink shadow-2xl flex flex-col h-[560px]">
              <div className="p-8 border-b border-ink flex items-center justify-between">
                <div>
                  <span className="text-[9px] font-black uppercase tracking-[0.3em] opacity-40">THIS WEEK · SNAPSHOTS</span>
              ...
              <ScrollArea className="flex-1">
                <div className="divide-y divide-ink/5">
                  ...
                  )) : <div className="p-20 text-center text-[10px] font-black uppercase opacity-20 italic">No updates recorded.</div>}
                </div>
              </ScrollArea>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-ink border border-ink shadow-2xl">
              {/* Card 1: Channel Performance (Horizontal Bar Chart) */}
              <div className="bg-white p-10 flex flex-col space-y-8 min-h-[500px]">
                  <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">CHANNEL PERFORMANCE</p>
              ...
              <div className="bg-white p-10 flex flex-col space-y-8 min-h-[500px]">
                  <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">SALES PIPELINE</p>
                      ...
                      {pipelineData.length > 0 ? pipelineData.map((stage, i) => (
                        <div key={i} className="space-y-2">
                           <div className="flex items-center justify-between text-[10px] font-black uppercase">
                              ...
                              <span className="opacity-40">{stage.value} RECORDS</span>
                           </div>
                           <div className="h-6 bg-foreground/[0.03] relative overflow-hidden">
                              ...
                              <div className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[9px] font-black opacity-40">
                                 {stage.percent.toFixed(1)}%
                              </div>
                           </div>
                        </div>
                      )) : <p className="p-20 text-center text-[10px] font-black uppercase opacity-20 italic">No lead data available.</p>}
                  </div>
              </div>
              ...
              <div className="bg-white p-10 flex flex-col space-y-8 min-h-[500px]">
                  <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">ACCOUNTABILITY PULSE</p>
                  ...
                  <div className="pt-6 border-t border-foreground/5 flex items-center justify-between">
                     <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[8px] font-black uppercase opacity-40 tracking-widest">Overdue</span>
                     </div>
                     <div className="flex items-center gap-1.5">
                        <div className="h-2 w-2 rounded-full bg-warning" />
                        <span className="text-[8px] font-black uppercase opacity-40 tracking-widest">Due Soon (72h)</span>
                     </div>
                  </div>
              </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 px-1"><Globe className="h-5 w-5 text-brand" /><h2 className="text-sm font-black uppercase tracking-[0.2em] opacity-40">Spends Insights</h2></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-ink border border-ink shadow-2xl">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
             <div className="xl:col-span-2 bg-white p-12 border border-ink shadow-2xl space-y-10">
                ...
                {sLoading ? (<div className="flex items-center gap-3 p-20 justify-center"><CircleNotch className="h-6 w-6 animate-spin text-brand" /><span className="text-[10px] font-black uppercase tracking-widest opacity-40">Updating AI analysis...</span></div>) : (<div className="whitespace-pre-wrap font-body text-[15px] leading-relaxed text-ink/80 border-l-[6px] border-brand pl-12 py-4">{snapshotDoc?.content || "Strategic analysis is being updated. Click 'REGENERATE' to refresh."}</div>)}
             </div>
             <div className="bg-cream p-10 border border-ink shadow-xl space-y-10">
                ...
                <div className="space-y-10">
                    <div className="space-y-4"><p className="text-[10px] font-black uppercase tracking-widest opacity-40">Operational Health (P-RAG)</p><HealthGrid green={snapshotDoc?.stats?.performanceRag?.Green || 0} amber={snapshotDoc?.stats?.performanceRag?.Amber || 0} red={snapshotDoc?.stats?.performanceRag?.Red || 0} /></div>
                    <div className="space-y-4"><p className="text-[10px] font-black uppercase tracking-widest opacity-40">Engagement Health (E-RAG)</p><HealthGrid green={snapshotDoc?.stats?.engagementRag?.Green || 0} amber={snapshotDoc?.stats?.engagementRag?.Amber || 0} red={snapshotDoc?.stats?.engagementRag?.Red || 0} /></div>
                    <div className="pt-10 border-t border-ink/10 space-y-6"><p className="text-[10px] font-black uppercase tracking-widest opacity-40">Major Shifts Detected</p>
                        {[...(snapshotDoc?.stats?.ragAdvancements || []), ...(snapshotDoc?.stats?.ragRisks || [])].length > 0 ? (<div className="space-y-5">{[...(snapshotDoc?.stats?.ragAdvancements || []), ...(snapshotDoc?.stats?.ragRisks || [])].slice(0, 8).map((shift, i) => (<div key={i} className="flex flex-col gap-2 p-4 bg-white border border-ink/5 group hover:border-brand/20 transition-colors"><div className="flex items-center justify-between gap-4"><div className="min-w-0"><p className="text-xs font-black uppercase truncate">{shift.brand}</p><p className="text-[8px] font-bold opacity-40 uppercase">{shift.team} • {shift.type}</p></div><div className={cn("text-[9px] font-mono font-black px-2 py-0.5 border h-fit", shift.direction === 'recovery' ? "bg-success/5 border-success/20 text-success" : "bg-destructive/5 border-destructive/20 text-destructive")}>{shift.from} → {shift.to}</div></div>{shift.reason && (<p className="text-[10px] leading-relaxed text-secondary italic border-l border-ink/10 pl-3 line-clamp-2">{shift.reason}</p>)}</div>))}</div>) : <p className="text-[10px] italic font-bold opacity-20">No significant shifts recorded WoW.</p>}
                    </div>
                </div>
             </div>
          </div>
  ...
  return (
    <div className="bg-white p-10 flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-2 gap-10">
            <div className="space-y-4"><span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-success"><ArrowUp className="h-3 w-3" /> TOP 3 GAINERS</span><div className="space-y-5">{gainers && gainers.length > 0 ? gainers.map((g, i) => (<div key={i} className="space-y-1"><p className="text-[14px] font-black text-ink leading-none truncate uppercase tracking-tight">{g.brand}</p><p className="text-[9px] font-bold text-secondary uppercase opacity-40">{g.type}</p><p className="text-[11px] font-black leading-none flex items-center justify-between text-success"><span>+{formatCurrency(g.amount || 0)}</span><span className="opacity-60 text-[9px]">({g.variance.toFixed(1)}%)</span></p></div>)) : <p className="text-[10px] font-bold text-secondary/20 italic">No significant gains</p>}</div></div>
            <div className="space-y-4"><span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-destructive"><ArrowDown className="h-3 w-3" /> TOP 3 LOSERS</span><div className="space-y-5">{losers && losers.length > 0 ? losers.map((l, i) => (<div key={i} className="space-y-1"><p className="text-[14px] font-black text-ink leading-none truncate uppercase tracking-tight">{l.brand}</p><p className="text-[9px] font-bold text-secondary uppercase opacity-40">{l.type}</p><p className="text-[11px] font-black leading-none flex items-center justify-between text-destructive"><span>{formatCurrency(l.amount || 0)}</span><span className="opacity-60 text-[9px]">({l.variance.toFixed(1)}%)</span></p></div>)) : <p className="text-[10px] font-bold text-secondary/20 italic">No significant losses</p>}</div></div>
      </div>
    </div>
  );
}
