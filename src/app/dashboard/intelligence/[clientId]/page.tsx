...
export default function ClientDeepDivePage() {
  ...
  if (isLoading) return <div className="flex flex-1 flex-col items-center justify-center p-20 gap-4"><Loader2 className="animate-spin h-12 w-12 text-primary" /><p className="text-[10px] font-black uppercase tracking-widest opacity-40">Synthesizing Strategic Pulse...</p></div>;

  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl glass" onClick={() => router.push('/dashboard/intelligence')}>
        ...
        <div className="flex items-center gap-3">
          ...
          <Select value={activeChannel} onValueChange={setActiveChannel}>
            <SelectTrigger className="h-10 w-48 rounded-2xl glass border-none text-[10px] font-black uppercase shadow-lg">
              <SelectValue placeholder="Channel" />
            </SelectTrigger>
            <SelectContent className="rounded-2xl glass border-none">
...
function ChartCard({ title, description, icon, children }: { title: string, description: string, icon: React.ReactNode, children: React.ReactNode }) {
  return (
    <Card className="glass-card border-none ios-shadow overflow-hidden group hover:shadow-2xl transition-all duration-500 min-h-[480px]">
      <CardHeader className="pb-2 pt-8 px-8">
        <div className="flex items-center gap-3 mb-1">
          <div className="h-10 w-10 rounded-2xl bg-foreground/5 flex items-center justify-center text-primary/40 group-hover:bg-primary/10 group-hover:text-primary transition-all duration-500">
            {icon}
          </div>
          <div>
            <CardTitle className="text-xl font-black font-headline tracking-tight">{title}</CardTitle>
            <CardDescription className="text-[10px] font-black uppercase tracking-widest opacity-40">{description}</CardDescription>
          </div>
        </div>
      </CardHeader>
...
function NoDataPlaceholder() {
  return (
    <div className="flex flex-col items-center justify-center h-full w-full space-y-3 opacity-20">
