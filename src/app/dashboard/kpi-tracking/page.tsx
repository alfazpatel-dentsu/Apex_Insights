...
function KpiTrackingContent() {
  ...
  const { data: registeredClients } = useCollection<Client>('clients');
  const { data: kpiDefinitions } = useCollection<Kpi>('kpiDefinitions');
  const { data: channelDefs } = useCollection<Channel>('channels');
  ...
  // Discover dropdown options from live KPI records when admin collections are empty/decommissioned
  const clients = useMemo<Client[]>(() => {
    const map = new Map<string, Client>();
    (registeredClients || []).forEach(c => {
      if (c.uniqueId) map.set(c.uniqueId, c);
    });
    (kpiData || []).forEach(k => {
      if (k.clientId && !map.has(k.clientId)) {
        map.set(k.clientId, {
          id: `discovered_${k.clientId}`,
          uniqueId: k.clientId,
          name: k.clientName || k.clientId,
          cluster: k.cluster,
          clusterLead: k.cduLead,
          emcsm: k.emCsm,
          subEntity: k.lob,
        });
      }
  ...
  }, [registeredClients, kpiData]);

  const discoveredKpis = useMemo<Kpi[]>(() => {
    const names = new Set<string>();
    (kpiDefinitions || []).forEach(k => { if (k.name) names.add(k.name); });
    (kpiData || []).forEach(k => { if (k.kpi) names.add(k.kpi); });
    return Array.from(names).sort().map(name => ({ id: name, name }));
  }, [kpiDefinitions, kpiData]);

  const discoveredChannels = useMemo<Channel[]>(() => {
    const names = new Set<string>();
    (channelDefs || []).forEach(c => { if (c.name) names.add(c.name); });
    (kpiData || []).forEach(k => { if (k.channel) names.add(k.channel); });
    return Array.from(names).sort().map(name => ({ id: name, name }));
  }, [channelDefs, kpiData]);

  const refreshWeeklyForKpis = async (ids: string[]) => {
    ...
    try {
      ...
      for (let i = 0; i < ids.length; i += 30) {
        ...
        const q = query(collection(firestore, 'kpiWeeklyData'), where('kpiDataId', 'in', chunk));
        const snap = await getDocs(q);
        snap.forEach(docSnap => { allWeekly.push({ id: docSnap.id, ...docSnap.data() } as KpiWeeklyData); });
      }
      setWeeklyData(prev => {
        const retained = prev.filter(w => !ids.includes(w.kpiDataId));
        return [...retained, ...allWeekly];
      });
    } catch (err) {
      console.error('Weekly data refresh failed:', err);
    }
  };
  ...
  const groupedDisplayData = useMemo(() => {
    ...
    return Object.values(groups).map(group => {
      ...
      const latestMonthRecord = Object.values(group.monthData).sort((a: any, b: any) => b.month.localeCompare(a.month))[0] as KpiData;
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
                      ...
                      return (
                          <TableRow key={group.id} className="border-b border-foreground/5 hover:bg-foreground/[0.02]">
                              ...
                              {weekDates.map((w) => {
                                  ...
                                  return (
                                      <TableCell key={`cell-${group.id}-${w.id}`} className="text-center p-1"><TooltipProvider><Tooltip><TooltipTrigger asChild><div className="flex items-center justify-center gap-1.5 group"><span className={cn("font-black text-[11px]", weeklyColor)}>{wd.achieved.toLocaleString()}</span><QuickCommentPopover weekData={wd} onSaved={(id, nextComment) => setWeeklyData(prev => prev.map(w => w.id === id ? { ...w, comment: nextComment } : w))} /></div></TooltipTrigger><TooltipContent className="rounded-xl glass border-none p-3 shadow-xl max-w-[200px]">{wd.comment && <div className="text-xs font-medium leading-relaxed">{wd.comment}</div>}</TooltipContent></Tooltip></TooltipProvider></TableCell>
                                  );
                              })}
                              <TableCell className="px-4 text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="rounded-xl glass border-none p-2"><DropdownMenuItem className="rounded-lg text-xs font-bold" onClick={() => { setSelectedKpiId(group.latestId); setIsDialogOpen(true); }}>Edit Latest Month</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell>
                          </TableRow>
                      );
                  })}
              </TableBody>
      ...
      <KpiDialog isOpen={isDialogOpen} onOpenChange={setIsDialogOpen} onSave={async (data) => {
        const { clientId, clientName, cluster, lob, cduLead, emCsm, channel, kpi, kpiType, currency, uploadRecordId, ...rest } = data;
        ...
        try {
          const savedId = await saveKpiData(firestore, { 
            month: format(dateRange?.to || new Date(), 'yyyy-MM'), 
            clientId: clientId || 'N/A', clientName, cluster: cluster || 'Unassigned', lob: lob || 'N/A', cduLead: cduLead || 'N/A', emCsm: emCsm || 'N/A', channel, kpi, kpiType: kpiType || 'PRIMARY', currency: currency || 'INR', 
          ...
          }, weeklyT.map((t, i) => ({ weekOfMonth: i + 1, target: t || 0, achieved: weeklyA[i] || 0, comment: weeklyC[i] || "" })), selectedKpiId || uploadRecordId || undefined);
          await refreshWeeklyForKpis([savedId]);
          setShouldFetch(true);
          toast({ title: selectedKpiId ? "KPI updated" : "KPI created" });
          setIsDialogOpen(false);
        } catch (e: any) {
          toast({ variant: "destructive", title: "Save failed", description: e.message || "Could not save KPI record." });
          throw e;
        }
      }} kpi={selectedKpi} weeklyData={selectedWeeklyData} currentMonth={format(dateRange?.to || new Date(), "MMMM yyyy")} weekDates={weekDates.filter(w => w.monthKey === format(dateRange?.to || new Date(), 'yyyy-MM')) as any} clients={clients} kpis={discoveredKpis} channels={discoveredChannels} />
...
export function QuickCommentPopover({ weekData, onSaved }: { weekData: KpiWeeklyData; onSaved?: (id: string, comment: string) => void }) {
    const firestore = useFirestore();
    ...
    useEffect(() => {
      setComment(weekData.comment || "");
    }, [weekData.comment, weekData.id]);
        ...
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            ...
            <PopoverContent className="w-[260px] p-4 rounded-2xl glass border-none shadow-2xl space-y-3" align="center" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex justify-end pt-1"><Button size="sm" className="h-8 rounded-lg font-bold text-[10px]" onClick={async () => {
                  setIsSaving(true);
                  try {
                    await updateWeeklyComment(firestore, weekData.id, comment);
                    onSaved?.(weekData.id, comment);
                    toast({ title: "Comment saved" });
                    setIsOpen(false);
                  } catch (e: any) {
                    toast({ variant: "destructive", title: "Save failed", description: e.message || "Could not save comment." });
                  } finally {
                    setIsSaving(false);
                  }
