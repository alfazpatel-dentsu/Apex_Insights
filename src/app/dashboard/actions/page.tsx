                  ...
                  <TableCell className="px-6 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="rounded-xl glass border-none shadow-2xl p-2 min-w-[160px]">
                        <DropdownMenuItem className="rounded-lg text-[10px] font-black uppercase tracking-widest gap-2" onClick={() => { setSelectedAction(action); setIsDialogOpen(true); }}>
                          <ArrowRight className="h-3 w-3" /> Edit Task
                        </DropdownMenuItem>
                        <DropdownMenuItem className="rounded-lg text-[10px] font-black uppercase tracking-widest text-destructive gap-2 focus:bg-destructive/10 focus:text-destructive" onClick={() => setDeletingId(action.id)}>
            ...
            <AlertDialogAction className="bg-destructive hover:bg-destructive/90 rounded-xl h-12 px-8 font-black uppercase text-[10px] tracking-widest" onClick={async () => {
              if (deletingId) {
                try {
                  ...
                  toast({ variant: 'destructive', title: 'Delete failed', description: e.message || 'Could not delete task.' });
                } finally {
                  setDeletingId(null);
                }
              }
