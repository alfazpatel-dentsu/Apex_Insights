...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        "shrink-0 h-screen sticky top-0 bg-surface border-r border-ink flex flex-col transition-[width] z-[100]",
        ...
        {filteredNav.map((item) => {
          ...
          return (
            ...
            >
              ...
              {mounted && !collapsed && <span className="truncate">{item.label}</span>}
            </Link>
          );
        })}
      </nav>
          ...
          <div className={cn("flex items-center gap-3", collapsed && "justify-center")}>
            ...
            {!collapsed && (
              <button
                ...
                title="Sign out"
                className="w-8 h-8 flex items-center justify-center border border-[#dcdcd4] hover:border-ink hover:bg-ink hover:text-cream transition-colors"
              >
                <SignOut size={13} />
              </button>
            )}
